...
Sube el archivo dentro de `news/`, con el nombre que quieras (por ejemplo
`news/2026-07-10-mazmorra-norte.json`) — el nombre del archivo no importa para nada, es solo para
que os aclaréis vosotros mismos en el repositorio. Lo único que importa es el contenido de dentro.
...
- Si subes la imagen al mismo repositorio de GitHub, la URL que necesitas es la de
  `raw.githubusercontent.com` (clic en la imagen dentro de GitHub → botón "Download" o "Raw" →
  copia esa URL), no el enlace normal a la página de GitHub que la muestra.
...
- ✅ `id` distinto al de cualquier otra noticia existente.
- ✅ `image` es la URL directa al archivo (raw), no el enlace a la página que lo muestra.
