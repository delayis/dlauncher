...
```json
[
  { "name": "2026-07-10-north-dungeon.json", "path": "news/2026-07-10-north-dungeon.json", "type": "file" },
  { "name": "2026-06-28-welcome.json", "path": "news/2026-06-28-welcome.json", "type": "file" }
]
```
...
## 3. How the image actually works (the part we don't do ourselves)
...
- **The images**: don't depend on GitHub at all, they're already host-agnostic (see point 3) —
...
- **The listing and content of `news/`**: this IS hardcoded to GitHub's API, in the `NEWS_DIR_URL`
  ...
  `fetch_remote_news()`. Moving *this* off GitHub does require code changes (see point 6).
...
## 6. What we'd need to change in the code to switch hosts
